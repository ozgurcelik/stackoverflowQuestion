...
def server_update(model, server_optimizer, server_state, weights_delta):
  ...
  lr = 1.0
  updated_model_weights = tf.nest.map_structure(lambda v,t: (v + (t*lr)), 
                                                model_weights, weights_delta)

  # Create a new state based on the updated model.
  return ServerState(
      model_weights=updated_model_weights,
...
@tf.function
def client_update(model, dataset, server_message, client_optimizer, client_eval_dataset):
  ...
  #tf.print(client_optimizer.weights)
  ...
  for batch in client_eval_dataset:
    preds = model.keras_model(batch['x'], training=False)
    #tf.print(preds[0])
    metric.update_state(y_true=batch['y'], y_pred=preds)
  accuracy = metric.result()
  # tf.print(accuracy)

  return ClientOutput(weights_delta, accuracy, loss_sum / client_weight)