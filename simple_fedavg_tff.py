...
from simple_fedavg_tf import get_model_weights
from simple_fedavg_tf import KerasModelWrapper
...
def build_federated_averaging_process(
  ...
  @tff.tf_computation(server_state_type, model_weights_type)
  ...
  @tff.tf_computation(tf_dataset_type, server_message_type, tf_dataset_type)
  def client_update_fn(tf_dataset, server_message, eval_dataset):
    model = model_fn()
    #client_optimizer = client_optimizer_fn()
    params = client_lr_schedule(server_message.round_num)
    client_optimizer = client_optimizer_fn(params)
    _initialize_optimizer_vars(model, client_optimizer)
    return client_update(model, tf_dataset, server_message, client_optimizer, eval_dataset)
  ...
  @tff.federated_computation(federated_server_state_type,
                             ...
                             federated_dataset_type)
  def run_one_round(server_state, federated_dataset, federated_eval_dataset):
    ...
    client_outputs = tff.federated_map(
        client_update_fn, (federated_dataset, server_message_at_client, federated_eval_dataset))
    ...
    return server_state, round_loss_metric, weight_denom
